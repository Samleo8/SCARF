...
class SceneDataset(Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        # fine-tuning setting ------------------------------------------------------------------
        if self.fine_tune:
            print('Dataloader set in fine-tune mode. Fine-tuning:',
                  self.fine_tune)
        ...
        if cfg.dataset_type == 'DTU':
            self.data, self.H, self.W, self.focal, self.cc, self.camera_system = DTU.setup_DTU(
                self.load_mode, cfg)
    ...
    def proj_pts_to_ref(self, pts, ref_poses):
        ...
        if self.cfg.dataset_type == 'DTU':
            for ref_pose in ref_poses:
                ref_pts.append(
                    [self.multi_world2cam(p.numpy(), ref_pose) for p in pts])
        else:
            for ref_pose in ref_poses:
                ref_pts.append([
    ...
    def proj_pts_to_ref_torch(self, pts, ref_poses, device, focal=None):
        ref_pts = torch.zeros(
            (len(ref_poses), pts.shape[0], pts.shape[1], 2)).to(device)

        if self.cfg.dataset_type == 'DTU':
            for i, ref_pose in enumerate(ref_poses):
                for j, p in enumerate(pts):
                    ref_pts[i, j] = self.multi_world2cam_torch(
                        p, ref_pose, device)
        else:
            for i, ref_pose in enumerate(ref_poses):
                for j, p in enumerate(pts):
                    ref_pts[i, j] = self.multi_world2cam_torch(
                        p, self.H, self.W, focal[0], ref_pose, device)
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.dataset_type == 'DTU':
            ...
            imgs, poses, poses_idx = DTU.load_scan_data(
                sample, self.load_mode, self.num_reference_views + 1, self.cfg,
                self.load_specific_reference_poses, self.load_fixed,
                self.shuffle)
            ref_images = imgs[:self.cfg.
                              num_reference_views]  # (num_ref_views, H, W, 3) np.array, f32
            ref_poses_idx = poses_idx[:self.cfg.
                                      num_reference_views]  # (num_reference_views) list, str
            ref_poses = poses[:self.cfg.
                              num_reference_views]  # (num_ref_views, 4, 4) np.array, f32
        ...
        ref_cam_locs = np.array([ref_pose[:3, 3] for ref_pose in ref_poses
                                 ])  # (num_ref_views, 3)
        rel_ref_cam_locs = ref_cam_locs - target_pose[:3,
                                                      3]  # (num_ref_views, 3)

        rays_o, rays_d = get_rays(self.H, self.W, self.focal, self.cc,
                                  torch.Tensor(target_pose),
                                  self.camera_system)  # (H, W, 3), (H, W, 3)
        output = {}

        # create relative reference view features
        self.ref_pose_features = [
            ref_pose[:3, 3] - target_pose[:3, 3] for ref_pose in ref_poses
        ]

        if self.mode == 'test':
            rays_o = torch.reshape(
                rays_o[::self.render_factor, ::self.render_factor], (-1, 3))
            rays_d = torch.reshape(
                rays_d[::self.render_factor, ::self.render_factor], (-1, 3))
            pts, z_vals = self.sample_ray(
                rays_o, rays_d
            )  # pts: (rays, num_samples, 3), z_vals: (rays, num_samples)
            ...
            if self.load_specific_rendering_pose is None:
                output['complete'] = [[
                    rays_o[i:i + N_rays_test], rays_d[i:i + N_rays_test],
                    viewdirs[i:i + N_rays_test], pts[i:i + N_rays_test],
                    z_vals[i:i + N_rays_test], ref_pts[:, i:i + N_rays_test],
                    ref_images, ref_poses, rel_ref_cam_locs, target, sample,
                    self.focal
                ] for i in range(0, rays_o.shape[0], N_rays_test)]
            else:
                output['complete'] = [[
                    rays_o[i:i + N_rays_test], rays_d[i:i + N_rays_test],
                    viewdirs[i:i + N_rays_test], pts[i:i + N_rays_test],
                    z_vals[i:i + N_rays_test], ref_pts[:, i:i + N_rays_test],
                    ref_images, ref_poses, rel_ref_cam_locs, sample, self.focal
                ] for i in range(0, rays_o.shape[0], N_rays_test)]
            ...
            coords_cropped = torch.stack(
                torch.meshgrid(torch.linspace(self.H // 2 - dH,
                                              self.H // 2 + dH - 1, 2 * dH),
                               torch.linspace(self.W // 2 - dW,
                                              self.W // 2 + dW - 1, 2 * dW),
                               indexing='ij'), -1)
            coords_full = torch.stack(
                torch.meshgrid(torch.linspace(0, self.H - 1, self.H),
                               torch.linspace(0, self.W - 1, self.W),
                               indexing='ij'), -1)  # (H, W, 2)

            for (name, coords) in [('cropped', coords_cropped),
                                   ('complete', coords_full)]:
                coords = torch.reshape(coords, [-1, 2])  # (H * W, 2)
                select_inds = np.random.choice(coords.shape[0],
                                               size=[N_rand],
                                               replace=False)  # (N_rand,)
                select_coords = coords[select_inds].long()  # (N_rand, 2)
                rays_o_selected = rays_o[select_coords[:, 0],
                                         select_coords[:, 1]]  # (N_rand, 3)
                rays_d_selected = rays_d[select_coords[:, 0],
                                         select_coords[:, 1]]  # (N_rand, 3)
                target_s = target[select_coords[:, 0],
                                  select_coords[:, 1]]  # (N_rand, 3)

                viewdirs = rays_d_selected / torch.norm(
                    rays_d_selected, dim=-1, keepdim=True)
                ...
                output[name] = [
                    rays_o_selected, rays_d_selected, viewdirs, target_s, pts,
                    z_vals, ref_pts, ref_images, rel_ref_cam_locs, ref_poses,
                    self.focal
                ]
    ...
    def sample_ray(self, rays_o, rays_d):
        ...
        near, far = self.near * torch.ones_like(
            rays_d[..., :1]), self.far * torch.ones_like(rays_d[..., :1])
...
def get_rays(H, W, focal, cc, c2w, camera_system):
    ...
    if camera_system == 'x_down_y_down_z_cam_dir':
        dirs = torch.stack([(i - cc[0]) / focal[0], (j - cc[1]) / focal[1],
                            torch.ones_like(i)], -1)
